...
cRenderCards::action_start();
	...
	cRender::button("show as table", cCommonFiles::server_filename()."?$gsTierQs");
...
cRenderCards::body_start();
	...
	foreach ($oResponse as $oExt){
		...
		$sMetric=cADTierMetricPaths::toTierCallsPerMin($oTier->name, $sTierTo);
		$aMetrics[] = [cChart::LABEL=>"Calls per min", cChart::METRIC=>$sMetric, cChart::GO_URL=>$sUrl, cChart::GO_HINT=>"Drill down"];
		$sMetric=cADTierMetricPaths::toTierResponseTimes($oTier->name, $sTierTo);
		$aMetrics[] = [cChart::LABEL=>"Response Times in ms", cChart::METRIC=>$sMetric];
		$sMetric=cADTierMetricPaths::toTierErrorsPerMin($oTier->name, $sTierTo);

'use strict'
/* globals bean,cQueueifVisible,cRenderQS,cBrowser,cHttp2, cChartConsts, google */
$.widget( "ck.adchart",{
	//#################################################################
	//# Definition
	//#################################################################
...
oOptions.pr__lower_div = oElement.attr("id") + "_L"
	...
	pr__get_url: function (){
		...
		if (oElement.attr(cChartConsts.ATTR_PREVIOUS) == 1) {
			...
			var sTitle = "(Previous) " + oElement.attr(cChartConsts.ATTR_TITLE+"0")
			oElement.attr(cChartConsts.ATTR_TITLE+"0", sTitle)
		}
	...
	pr__draw_chart: function(poJson){
		...
		var oChart, dDate, iValue, iItemMax, iMax, iMaxObserved, iAvgObs, iSum, sMax, iMin
		...
		oButton = $("<button>",{class:"csv_button",title:"Zoom"}).button({icon:"ui-icon-arrow-4-diag"})
			oButton.click(		function(){ oThis.onClickZoom()}		)
		oSpan.append(oButton)
			
		oButton = $("<button>",{class:"csv_button",title:"Compare"}).button({icon:"ui-icon-shuffle"})
			oButton.click(		function(){ oThis.onClickCompare()}		)
		oSpan.append(oButton)
		
		oButton = $("<button>",{class:"csv_button",title:"Statistics"}).button({icon:"ui-icon-signal"})
		...
		if (oTopElement.attr(cChartConsts.ATTR_GO_URL)){
			...
			oButton = $("<button>",{class:"csv_button",title:sGoLabel}).button({icon:"ui-icon-arrowreturn-1-n"})

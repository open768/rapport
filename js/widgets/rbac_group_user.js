...
$.widget( "ck.adrbacgroupusers",{
	...
	_create: function(){
		...
		if (!oElement.attr(cRender.GROUP_NAME_QS))		$.error("group name missing!");
		if (!oElement.attr(cRender.GROUP_ID_QS))		$.error("group id missing!");
	...
	get_url: function (){
		...
		var oParams = {};
		oParams[ cRender.GROUP_NAME_QS ] = oElement.attr(cRender.GROUP_NAME_QS);
		oParams[ cRender.GROUP_ID_QS ] = oElement.attr(cRender.GROUP_ID_QS);
